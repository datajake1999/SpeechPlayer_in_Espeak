...
{//=====================================
	...
	len_string[0] = 0;
	param_string[0] = 0;
	if(p->ph->std_length > 0)
        sprintf(len_string,"  Length %d", p->ph->std_length*2);
	if(p->ph->std_length != 0)
		sprintf(param_string,", %d", p->ph->std_length);

	sprintf(buf,"Stress %s%d   Amp %2d   LengthMod %2d   Pitch %3d %3d  %s    PhFlags %.2x    (%s%s)%s",
		emphasized,p->stresslevel&0x7,p->amp, p->length,y1,y2,name,p->ph->phflags, LookupManifest(p->ph->std_length), param_string, len_string);
