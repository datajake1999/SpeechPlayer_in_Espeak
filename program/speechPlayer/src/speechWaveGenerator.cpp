...
class CascadeFormantGenerator { 
	...
	double getNext(const speechPlayer_frame_t* frame, bool glottisOpen, double input) {
		input/=2.0;
		double n0Output=(rN0.resonate(input,frame->cfN0,frame->cbN0)*frame->caN0);
		double output=calculateValueAtFadePosition(input,rNP.resonate(n0Output,frame->cfNP,frame->cbNP),frame->caNP);
		output+=(r1.resonate(output,frame->cf1+frame->dcf1,frame->cb1+frame->dcb1)-output)*frame->ca1;
		output+=(r2.resonate(output,frame->cf2,frame->cb2)-output)*frame->ca2;
		output+=(r3.resonate(output,frame->cf3,frame->cb3)-output)*frame->ca3;
		output+=(r4.resonate(output,frame->cf4,frame->cb4)-output)*frame->ca4;
		output+=(r5.resonate(output,frame->cf5,frame->cb5)-output)*frame->ca5;
		output+=(r6.resonate(output,frame->cf6,frame->cb6)-output)*frame->ca6;
		return output;
	}
